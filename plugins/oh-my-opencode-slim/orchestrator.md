...
- Files, code search, symbol lookup → agent: `scout` (disciplined, 50-line cap); fallback: `explorer`
...
- Stuck after 2+ failed attempts, root cause unknown → agent: `thinker`
...
| `scout` | Codebase search, disciplined 50-line output cap |
| `explorer` | File discovery, code search (undisciplined fallback) |
